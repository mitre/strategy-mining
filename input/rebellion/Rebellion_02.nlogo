...
globals [
  ...
  num-cops            ; number of cops
  active-list         ; list of number of active agents
]
...
to setup
  ...
  set num-agents round (initial-agent-density * .01 * count patches)
  set num-cops round (initial-cop-density * .01 * count patches)
  ...
  ; create agents
  nw:generate-small-world agents links round sqrt num-agents round sqrt num-agents 2.0 false [
      move-to one-of patches with [ not any? turtles-here ]
      set heading 0
...
to go
  ...
  set active-list lput count agents with [ active? ] active-list
  tick
end
...
to-report num-active
  report count agents with [ active? ]
end
